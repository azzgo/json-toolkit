...
- **When** 页面加载完成
- **Then** 所有界面元素使用新的色彩变量：
  - 主背景色为纯白色 (`oklch(1 0 0)`)
  - 卡片背景色为浅灰色 (`oklch(0.98 0 0)`)
  - 主色调为技术蓝 (`oklch(0.55 0.15 244)`)
  - 文字色为深灰 (`oklch(0.15 0.01 264)`) 而非纯黑
  - 边框使用明显灰色 (`oklch(0.45 0.02 264)`) 确保可见性
  - 输入框背景为纯白色，与页面背景形成对比
...
### Requirement: 表单控件设计一致性
系统MUST确保所有输入控件具有一致和清晰的视觉设计。

#### Scenario: 输入框可见性
- **Given** 任何输入框或选择框组件
- **When** 组件渲染在页面上
- **Then** 输入框具有明显的深色边框 (`oklch(0.45 0.02 264)`)
- **And** 边框宽度为2px，确保在非聚焦状态下清晰可见
- **And** 背景色为纯白色 (`oklch(1 0 0)`)，与页面背景形成对比
- **And** 在所有状态（非聚焦、悬停、聚焦）下都有清晰的边界

#### Scenario: 输入框交互状态
- **Given** 用户与输入框交互
- **When** 鼠标悬停时
- **Then** 边框颜色变为主色调的30%透明度
- **When** 输入框获得焦点时
- **Then** 边框颜色变为主色调，并显示光环效果
- **When** 输入框失去焦点时
- **Then** 边框恢复为默认深色，保持清晰可见

#### Scenario: 禁用状态区分
- **Given** 被禁用的输入框
- **When** 组件渲染
- **Then** 背景色使用静默色 (`--muted`)
- **And** 文字颜色变为静默前景色 (`--muted-foreground`)
- **And** 明确区分于正常可编辑状态
...
- **Given** JSON差异比较视图
- **When** 显示有变更的行
- **Then** 删除的内容使用柔和红色背景 (`bg-red-50 dark:bg-red-950/30`)
- **And** 新增的内容使用柔和绿色背景 (`bg-green-50 dark:bg-green-950/30`)
- **And** 修改的内容使用柔和橙色背景 (`bg-orange-50 dark:bg-orange-950/30`)
